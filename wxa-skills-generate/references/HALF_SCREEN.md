...
> 半屏页面是**原子组件内容的延伸**，**非必要流程**。仅当业务上需要"展示更多详情"或"让用户补充信息"时，才在原子组件内的某个按钮上挂 tap handler 调起。
>
> **默认不生成**。Skill 生成阶段不要主动给业务做出"挂半屏"的设计；只有用户明确要求或源页面已有"详情/补充信息表单"等强烈半屏语义时才参考本指南。
...
- [ ] 仅在源业务**确有"详情/补充信息"语义**时才挂半屏，否则不生成
