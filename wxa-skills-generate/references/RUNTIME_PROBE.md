...
> 阶段 3「接口与 JSAPI 提取」**按需**触发。当源码静态分析无法得出可靠结论时，使用 [`miniprogram-automator`](https://developers.weixin.qq.com/miniprogram/dev/devtools/auto/automator.html) 启动微信开发者工具、在**源项目页面**上模拟用户交互、捕获**真实**的网络请求与响应，作为阶段 4 设计原子接口的依据。
>
...
| 项 | 要求 |
|---|------|
| `miniprogram-automator` | 已通过 `npm i -D miniprogram-automator` 安装到项目根 |
| 开发者工具 CLI | macOS 默认 `/Applications/wechatwebdevtools.app/Contents/MacOS/cli`；Win 默认 `C:/Program Files (x86)/Tencent/微信web开发者工具/cli.bat`；支持环境变量 `WX_CLI_PATH` 和自动搜索 |
| CLI/HTTP 端口 | 工具内「设置 → 安全设置 → 服务端口」开启 |
| `auto-port` | 默认 `9420`，被占用时换端口 |

任一项不满足 → 告知用户具体修复步骤，**不要尝试代为开启**。
...
1. **优先用 probe 验证**——对 outputSchema 的准确性至关重要，宁可多跑一次 automator，也不要在阶段 5 猜字段名导致 validator 反复修补
2. **起就一次起完**——一次性把所有需要探测的接口列入 plan 批量跑，不要反复启停
3. **probe 失败不等于阻断**——可以降级到离线兜底，只有「静态 + probe + 离线兜底全失败」才走阻断规则 B
4. **探测的是源项目，不是 skills/**——这一步在生成 skills/ 之前，与已生成的分包无关
5. **非阻断通知**——一次性告知用户将启动 automator，不等待确认；仅在环境检查失败时中断
